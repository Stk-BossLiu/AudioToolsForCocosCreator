...
var SpectrogramPlugin = /** @class */ (function (_super) {
    __extends(SpectrogramPlugin, _super);
    function SpectrogramPlugin(options) {
        var _this = _super.call(this, options) || this;
        _this.frequenciesDataUrl = options.frequenciesDataUrl;
        _this.container =
        ...
        if (options.colorMap && typeof options.colorMap !== 'string') {
            ...
            for (var i = 0; i < options.colorMap.length; i++) {
                var cmEntry = options.colorMap[i];
                if (cmEntry.length !== 4) {
            ...
            _this.colorMap = options.colorMap || 'roseus';
            switch (_this.colorMap) {
                case 'gray':
                    _this.colorMap = [];
                    for (var i = 0; i < 256; i++) {
                        var val = (255 - i) / 256;
    ...
    SpectrogramPlugin.prototype.onInit = function () {
        ...
        this.subscriptions.push(this.wavesurfer.on('redraw', function () { return _this.render(); }));
    };
    ...
    SpectrogramPlugin.prototype.loadFrequenciesData = function (url) {
    ...
    SpectrogramPlugin.prototype.createWrapper = function () {
        this.wrapper = (0, dom_js_1.default)('div', {
            style: {
                display: 'block',
                position: 'relative',
                userSelect: 'none',
        ...
        if (this.options.labels) {
            this.labelsEl = (0, dom_js_1.default)('canvas', {
                part: 'spec-labels',
                style: {
                    position: 'absolute',
                    zIndex: 9,
    ...
    SpectrogramPlugin.prototype.createCanvas = function () {
        this.canvas = (0, dom_js_1.default)('canvas', {
            style: {
                position: 'absolute',
                left: 0,
                top: 0,
    ...
    SpectrogramPlugin.prototype.render = function () {
        var _a;
        if (this.frequenciesDataUrl) {
            this.loadFrequenciesData(this.frequenciesDataUrl);
        }
        else {
            var decodedData = (_a = this.wavesurfer) === null || _a === void 0 ? void 0 : _a.getDecodedData();
            if (decodedData) {
                this.drawSpectrogram(this.getFrequencies(decodedData));
            }
        }
    };
    SpectrogramPlugin.prototype.drawSpectrogram = function (frequenciesData) {
        ...
        var _loop_1 = function (c) {
            // for each channel
            var pixels = this_1.resample(frequenciesData[c]);
            var imageData = new ImageData(width, height);
            for (var i = 0; i < pixels.length; i++) {
                for (var j = 0; j < pixels[i].length; j++) {
                    var colorMap = this_1.colorMap[pixels[i][j]];
    ...
    SpectrogramPlugin.prototype.hzToMel = function (hz) { return 2595 * Math.log10(1 + hz / 700); };
    SpectrogramPlugin.prototype.melToHz = function (mel) { return 700 * (Math.pow(10, mel / 2595) - 1); };
    SpectrogramPlugin.prototype.createMelFilterBank = function (numMelFilters, sampleRate) {
        var _this = this;
        var melMin = this.hzToMel(0);
        var melMax = this.hzToMel(sampleRate / 2);
        var melFilterBank = Array.from({ length: numMelFilters }, function () { return Array(_this.fftSamples / 2 + 1).fill(0); });
        var scale = (sampleRate / this.fftSamples);
        for (var i = 0; i < numMelFilters; i++) {
            var hz = this.melToHz(melMin + (i / numMelFilters) * (melMax - melMin));
    ...
    SpectrogramPlugin.prototype.applyMelFilterBank = function (fftPoints, melFilterBank) {
        var numFilters = melFilterBank.length;
        var melSpectrum = Array(numFilters).fill(0);
    ...
    SpectrogramPlugin.prototype.getFrequencies = function (buffer) {
        ...
        var channels = ((_a = this.options.splitChannels) !== null && _a !== void 0 ? _a : (_b = this.wavesurfer) === null || _b === void 0 ? void 0 : _b.options.splitChannels) ? buffer.numberOfChannels : 1;
        ...
        var melFilterBank = this.createMelFilterBank(this.numMelFilters, sampleRate);
        for (var c = 0; c < channels; c++) {
            ...
            while (currentOffset + fftSamples < channelData.length) {
                ...
                for (var j = 0; j < fftSamples / 2; j++) {
                    ...
                    var valueDB = 20 * Math.log10(spectrum[j]);
    ...
    SpectrogramPlugin.prototype.freqType = function (freq) {
        return freq >= 1000 ? (freq / 1000).toFixed(1) : Math.round(freq);
    };
    SpectrogramPlugin.prototype.unitType = function (freq) {
        return freq >= 1000 ? 'KHz' : 'Hz';
    };
    SpectrogramPlugin.prototype.loadLabels = function (bgFill, fontSizeFreq, fontSizeUnit, fontType, textColorFreq, textColorUnit, textAlign, container, channels) {
    ...
    SpectrogramPlugin.prototype.resample = function (oldMatrix) {
        ...
        for (i = 0; i < columnsNumber; i++) {
            ...
            for (j = 0; j < oldMatrix.length; j++) {
                ...
                var overlap = Math.max(0, Math.min(oldEnd, newEnd) - Math.max(oldStart, newStart));
                var k 
